...
public class GroupInfoPacket extends Packet {
	...
	public Group getGroup(String longId) {
		...
		try {
			group = new GroupImpl(api, new JSONObject(data).getString("id"));
		...
		try {
			...
			for (int ii = 0; ii < membersArray.length(); ii++) {
				...
				User optional = api.getClient().getUser(member.getString("id").split("8:")[1]);

				if (optional != null) {
					if (!member.getString("role").equals("User"))
						role = GroupUser.Role.MASTER;

					group.getClients().add(new GroupUser(optional, role, group));
				}
			}
		} catch (Exception e) {
			...
			for (int ii = 0; ii < membersArray.length(); ii++) {
				...
				try {
					...
					if (optional != null) {
						...
						groupMembers.add(new GroupUser(optional, role, new GroupImpl(api, id)));
					}
				} catch (Exception e) {
